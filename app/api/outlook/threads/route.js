...
export async function GET(request) {
  ...
  if (!outlookToken) return NextResponse.json({ error: 'Outlook not connected', connected: false }, { status: 200 })
  ...
  const data = await res.json()
  if (!res.ok) return NextResponse.json({ error: 'Failed to fetch messages', connected: true }, { status: 500 })
