...
/** Left column of the Create Ticket page — conversations we initiated that are
 *  awaiting the customer's reply (status `pending`). They auto-move to the Inbox
 *  (status `open`) once the customer responds. Empty when the workspace has none. */
export function CreateTicketRecentPanel({ onBack }: CreateTicketRecentPanelProps) {
  const { data: threads = [], isLoading } = useConversations("pending", "");
      ...
      <div className="shrink-0 border-b border-border px-3.5 pb-2.5 pt-3.5">
        <div className="mb-2.5 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="rounded border border-border bg-secondary px-1.5 py-px text-[10px] font-semibold text-foreground-2">New ticket</span>
          </div>
        ...
        <div className="text-[9.5px] font-bold uppercase tracking-[0.1em] text-muted-foreground">Recent tickets</div>
      </div>

      {/* Recent tickets — real conversations, empty when none */}
      <div className="flex-1 overflow-y-auto">
        {!isLoading && recent.length === 0 && <div className="px-5 py-10 text-center text-[11.5px] text-muted-foreground">No recent tickets</div>}
        {recent.map((t: Thread) => {
          const name = t.customer_name || extractName(t.from);
          return (
            <div key={t.id} className="flex cursor-pointer items-start gap-[9px] border-b border-border px-3.5 py-2.5 transition-colors hover:bg-secondary">
