-- Fix: admins/owners never got a Stop button when their open session started
-- before the current filter window.
--
-- The admin branch of api_list_time_sessions returned a DATE-WINDOWED `sessions`
-- list but no `active_session` field. The frontend (time-tracking-page.tsx)
-- reconstructs the logged-in user's live session for admins by scanning that
-- windowed list: `sessions.find(s => s.agent_id === myUid && !s.clocked_out_at)`.
-- Any still-open session clocked in before v_from (e.g. before this week's
-- Monday) is excluded from `sessions`, so the find returns nothing, the timer
-- stays on "Start", yet `active_count` (which is NOT date-filtered) still reads
-- 1 — the inconsistent/"broken" state. api_time_clock_in meanwhile returns
-- `already_active: true`, so clicking Start creates nothing.
--
-- Fix: the admin branch now returns an explicit `active_session`, computed with
-- NO date filter, exactly mirroring the employee branch. `data.active_session`
-- becomes authoritative for both roles; the frontend's windowed `.find()`
-- fallback stops mattering. Only api_list_time_sessions is redefined; all other
-- time RPCs are unchanged.
