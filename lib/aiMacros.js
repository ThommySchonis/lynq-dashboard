...
2. Naming convention: "${answers.store_name ?? ''} | {CATEGORY} - {Short descriptive name}"
   Example: "${answers.store_name ?? 'Acme'} | TRK - Tracking Number Provided"
   The store name prefix is mandatory on every macro so the same workspace can later identify which store a macro belongs to.
...
8. Length: each macro 80-150 words MAX (excluding HTML tags). Aim for 80-120 typical. Stay concise — quality and clarity over verbose paragraphs.
...
{
  "macros": [
    { "name": "${answers.store_name ?? 'Acme'} | TRK - Tracking Number Provided", "body": "<p>Hi {{ticket.customer.firstname}},</p><p>...</p>", "tags": ["tracking", "shipping", "status"] },
    ... 49 more
  ]
}
