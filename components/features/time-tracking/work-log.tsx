  ...
  return (
    <div className="overflow-hidden rounded-[10px] border border-black/7 bg-white animate-fade-in-3">
      <div className="border-b border-black/6 px-4.5 py-3.5">
        <div className="text-[13px] font-semibold text-[#0F0F10]">Work Log</div>
      ...
      {sessions.length === 0 ? (
        ...
        <>
          ...
          {sessions.map(s => (
            <div
              key={s.id}
              className="grid grid-cols-[130px_140px_70px_1fr] gap-3 border-b border-black/5 px-4.5 py-2.5 transition-colors last:border-b-0 hover:bg-[#F9F8FF]"
            >
              <div className="text-xs font-medium text-[#0F0F10]">{fmtDate(s.clocked_in_at)}</div>
              ...
              <div className="text-[13px] font-semibold text-[#0F0F10] tabular-nums">{fmtDur(durSec(s))}</div>
