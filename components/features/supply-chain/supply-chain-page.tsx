...
export function SupplyChainPage() {
  ...
  // Not-configured state renders the full-bleed Parcel Panel setup wizard.
  if (notConfigured) {
    return <SetupWizard onConnected={() => void refetch()} />;
  }

  return (
    <main
      className="min-h-screen overflow-y-auto py-10 px-11 relative text-foreground [&::-webkit-scrollbar]:w-[3px] [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-(--scrollbar) [&::-webkit-scrollbar-thumb]:rounded-sm"
      style={{ fontFamily: "'Switzer', -apple-system, BlinkMacSystemFont, sans-serif" }}
    >
      <div className="relative z-[1] max-w-[980px] mx-auto">
        {/* Header */}
        <div className="animate-[fadeUp_0.4s_ease_both] mb-[30px]">
          <div className="flex items-start justify-between gap-4">
            <div>
              <div className="flex items-center gap-2.5 mb-2.5">
                ...
                <span className="text-[11.5px] font-bold text-muted-foreground uppercase tracking-[.08em]">Supply Chain</span>
              </div>
              <h1 className="text-xl font-bold tracking-[-0.02em] leading-[1.1] mb-2 text-foreground">Shipment Tracker</h1>
          ...
          <div className="h-px bg-secondary mt-[22px]" />
        </div>
        ...
        {!isPending && !isError && (
          <>
            ...
            {/* Rows or empty state */}
            {filtered.length === 0 ? (
              <div className="bg-card border border-border rounded-2xl shadow-[var(--shadow-card)] text-center py-10 px-5 text-muted-foreground text-[13.5px]">
                No shipments match this filter.
              </div>
            ) : (
              <div className="flex flex-col gap-2.5">
                {filtered.map((order, i) => (
                  <ShipmentRow key={order.order_number || i} order={order} index={i} isAttention={attentionKeys.has(order.order_number || order.id)} />
                ))}
              </div>
            )}
          </>
