...
export function SettingsSidebar() {
  ...
  return (
    <aside className="fixed left-[208px] top-0 bottom-0 w-[260px] bg-[#F8F7FA] border-r border-[#E5E0EB] flex flex-col z-40">
      {/* Header */}
      <div className="px-4 pt-5 pb-4 border-b border-[#E5E0EB] flex-shrink-0">
        <h2 className="text-[22px] font-semibold text-[#1C0F36] mb-3 mt-0 leading-tight">
          Settings
        </h2>

        <div className="relative" ref={searchRef}>
          {/* Search icon */}
          <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[#9B91A8] flex items-center pointer-events-none z-10">
          ...
          <Input
            ...
            autoComplete="off"
            className="pl-8 pr-10 py-2 text-[13px] bg-white border-[#E5E0EB] text-[#1C0F36] placeholder:text-[#9B91A8] focus-visible:ring-[#A175FC]/20 focus-visible:border-[#A175FC]"
          />

          {/* ⌘K hint badge */}
          <span className="absolute right-2 top-1/2 -translate-y-1/2 text-[10px] text-[#9B91A8] bg-[#F0EDF4] rounded-[3px] px-1 leading-[1.6] pointer-events-none">
            ⌘K
          </span>
          ...
          {dropdownOpen && filtered.length > 0 && (
            ...
            >
              {filtered.map(item => (
                <div
                  key={item.href}
                  className="flex flex-col gap-px py-[9px] px-3 cursor-pointer text-[13px] text-[#1C0F36] transition-colors hover:bg-[#F8F7FA]"
                  onMouseDown={() => handleDropdownItemClick(item.href)}
                >
                  <span>{item.label}</span>
                  <span className="text-[10px] text-[#9B91A8] font-medium tracking-[0.04em]">
                    {item.group}
                  </span>
                </div>
              ))}
          ...
          {dropdownOpen && filtered.length === 0 && query.trim().length > 0 && (
            ...
            >
              <p className="py-3 px-3 text-[13px] text-[#9B91A8] text-center">
      ...
      <nav className="flex-1 overflow-y-auto py-2 pb-5 [scrollbar-width:thin] [scrollbar-color:#E5E0EB_transparent] [&::-webkit-scrollbar]:w-[3px] [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-[#E5E0EB] [&::-webkit-scrollbar-thumb]:rounded-sm">
        {SETTINGS_NAV.map((group, groupIdx) => (
          <div key={group.label}>
            ...
            <div className={`px-4 pb-1.5 text-[11px] font-semibold uppercase tracking-[0.08em] text-[#9B91A8] select-none ${groupIdx === 0 ? 'pt-3' : 'pt-5'}`}>
              {group.label}
            </div>
              ...
              <div key={item.href} className="px-2 py-0.5">
                <Link
                  ...
                  className={[
                    ...
                    pathname === item.href
                      ? 'bg-[#EDE5FE] text-[#A175FC] font-medium'
                      : 'text-[#6B5E7B] font-normal hover:bg-black/[0.04] hover:text-[#1C0F36]',
                    'focus-visible:outline-2 focus-visible:outline-[#A175FC] focus-visible:outline-offset-[-2px]',
                  ].join(' ')}
                >
                  {item.label}
                </Link>
