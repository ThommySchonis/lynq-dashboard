...
import type { ShopifyCredentials } from "./shopify-types.ts";
...
export async function getOrders(credentials: ShopifyCredentials, options: { limit?: number } = {}) {
  ...
  return data.orders.edges.map(({ node }) => ({
    ...
    name: node.name,
    customer: node.customer ? `${node.customer.firstName ?? ""} ${node.customer.lastName ?? ""}`.trim() : "Unknown",
...
function aggregateRefunds(orders: RefundAggOrder[], dateRange: { from: string; to: string }) {
  ...
  return orders
    ...
    .flatMap((o) => {
      ...
      const refundTotal = inRange.reduce((sum, r) => sum + r.transactions.reduce((ts, t) => ts + parseFloat(t.amount || "0"), 0), 0);
      ...
      return [
        {
          orderId: o.name,
          orderIdNumeric: o.id,
          customer: o.customer ? `${o.customer.firstName || ""} ${o.customer.lastName || ""}`.trim() || "Unknown" : o.email || "Unknown",
...
export async function syncOrders(workspaceId: string, credentials: ShopifyCredentials, userId: string, options: { full?: boolean; storeId?: string } = {}) {
  ...
  const query = options.full ? "" : `processed_at:>='${new Date(Date.now() - 90 * 24 * 60 * 60 * 1000).toISOString()}'`;
  ...
  const rows = orders.map((order) => {
    ...
    const refundAmount = order.refunds.reduce(
      (sum, r) => sum + r.transactions.edges.reduce((ts, { node: t }) => ts + parseFloat(presentmentAmount(t.amountSet)), 0),
      0,
    );
