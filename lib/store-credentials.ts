...
export async function getStoreCredentials(
  ...
  // Non-expiring / manual token — return as-is
  ...
  // Refresh token missing or expired — store must reconnect
  ...
  // Persist new tokens — if DB update fails, still return the valid new token
  await supabaseAdmin
    .from('integrations')
    .update({
