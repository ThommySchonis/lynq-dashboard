...
import { computeNextRetryAt, MAX_ATTEMPTS } from './webhook-retry.ts'
...
export async function withIdempotency(options: WithIdempotencyOptions): Promise<Response> {
  ...
  if (!eventId) {
    ...
    try {
      ...
      logger.error(`[webhook/${source}]`, 'handler error (no dedup)', {
        error: err instanceof Error ? err.message : String(err),
      })
      return jsonRes({ error: 'Handler failed' }, 500)
    }
  }
  ...
  if (insertError) {
    ...
    if (insertError.code === '23505') {
      ...
      if (!existing) {
        ...
        return jsonRes({ error: 'Transient conflict' }, 500)
      }
      ...
      if (existing.status === 'failed') {
        ...
        const { error: retryInsertError } = await sb
          .from('webhook_events')
          .insert({
            event_id: eventId,
            source,
