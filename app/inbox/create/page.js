...
const DEMO_RECENT = [
  ...
  { id:'r5', from:'Anna Smit',        subject:'Exchange size — order #1045', time:'3d ago',  status:'resolved' },
]
const STATUS_CFG = {
  open:     { color:'#7C5CFC', label:'Open'     },
...
// ─── CSS — follows user theme (light/dark via CSS vars) ──────
const CSS = `
  ...
  @keyframes auroraA { 0%,100%{transform:translate(0,0) scale(1);opacity:.6} 33%{transform:translate(70px,-90px) scale(1.28);opacity:.8} 66%{transform:translate(-50px,45px) scale(.88);opacity:.5} }
  ...
  button:focus-visible  { outline:2px solid rgba(124,92,252,0.5); outline-offset:2px; border-radius:6px; }
  input:focus-visible, [contenteditable]:focus-visible { outline:none; }

  /* ── Left panel ── */
  .ct-panel { background:var(--bg-surface); border-right:1px solid var(--border); }
  [data-theme="dark"] .ct-panel { background:rgba(10,4,28,0.52); backdrop-filter:blur(24px); -webkit-backdrop-filter:blur(24px); border-right-color:rgba(255,255,255,0.07); }

  /* ── Thread rows ── */
  .ct-trow { padding:10px 14px; cursor:pointer; border-bottom:1px solid var(--border); display:flex; align-items:flex-start; gap:9px; transition:background .14s; }
  .ct-trow:hover { background:var(--bg-surface-2); }

  /* ── Compose area ── */
  .ct-compose { background:var(--bg-surface); }
  [data-theme="dark"] .ct-compose { background:#160c35; }

  .ct-compose-bottom { background:var(--bg-surface); }
  [data-theme="dark"] .ct-compose-bottom { background:#120931; }

  /* ── Compose editor ── */
  .ct-body { width:100%; resize:none; outline:none; font-family:inherit; background:transparent; border:none; padding:14px 16px; font-size:13.5px; color:var(--text-1); line-height:1.78; letter-spacing:.005em; }
  .ct-body[contenteditable=true]:empty:before { content:attr(data-placeholder); color:var(--text-3); pointer-events:none; display:block; }

  /* ── Toolbar buttons ── */
  .ct-rb { min-width:30px; height:30px; display:flex; align-items:center; justify-content:center; border-radius:7px; font-size:12px; font-weight:700; font-family:inherit; color:var(--text-3); transition:all .15s; border:none; background:transparent; padding:0 6px; }
  .ct-rb:hover { background:var(--bg-surface-2); color:var(--text-1); }

  /* ── Macro dropdown ── */
  .ct-mdd { position:absolute; bottom:calc(100% + 4px); left:0; right:0; background:var(--bg-surface); border:1px solid var(--border); border-radius:10px; box-shadow:0 -12px 32px rgba(0,0,0,0.1); z-index:60; max-height:220px; overflow-y:auto; padding:4px; animation:fadeUp .14s ease both; }
  [data-theme="dark"] .ct-mdd { background:#1a0b3d; box-shadow:0 -12px 32px rgba(0,0,0,0.4); }
  ...
  .ct-mb:hover { background:var(--bg-surface-2); }

  /* ── Demo notice bar ── */
  .ct-demo-bar { display:flex; align-items:center; gap:7px; padding:7px 16px; border-bottom:1px solid var(--border); background:rgba(217,119,6,0.05); }
  [data-theme="dark"] .ct-demo-bar { background:rgba(251,191,36,0.06); }

  /* ── Input rows ── */
  .ct-row { display:flex; align-items:center; padding:9px 16px; border-bottom:1px solid var(--border); gap:8px; }

  /* ── Aurora (dark only) ── */
  .ct-al1, .ct-al2, .ct-grid { display:none; }
  [data-theme="dark"] .ct-al1 { display:block; position:absolute; top:-25%; left:12%; width:900px; height:800px; border-radius:50%; background:radial-gradient(ellipse,rgba(161,117,252,0.52) 0%,rgba(124,58,237,0.24) 38%,transparent 65%); animation:auroraA 22s ease-in-out infinite; filter:blur(60px); pointer-events:none; }
  [data-theme="dark"] .ct-al2 { display:block; position:absolute; top:2%; left:3%; width:400px; height:400px; border-radius:50%; background:radial-gradient(ellipse,rgba(139,92,246,0.38) 0%,rgba(109,40,217,0.14) 50%,transparent 72%); animation:auroraD 19s ease-in-out infinite; filter:blur(45px); pointer-events:none; }
  [data-theme="dark"] .ct-grid { display:block; position:absolute; inset:0; background-image:linear-gradient(rgba(255,255,255,0.015) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,0.015) 1px,transparent 1px); background-size:72px 72px; pointer-events:none; }

  /* ── Scrollbar ── */
  .ct-wrap ::-webkit-scrollbar { width:3px; }
  .ct-wrap ::-webkit-scrollbar-track { background:transparent; }
  .ct-wrap ::-webkit-scrollbar-thumb { background:var(--scrollbar,rgba(0,0,0,0.12)); border-radius:2px; }

  /* ── Skel loading ── */
  .ct-skel { background:linear-gradient(90deg,var(--skeleton-from,rgba(0,0,0,0.05)) 25%,var(--skeleton-to,rgba(0,0,0,0.08)) 50%,var(--skeleton-from,rgba(0,0,0,0.05)) 75%); background-size:400% 100%; animation:shimmer 1.6s linear infinite; border-radius:5px; }
`
...
function Toast({ msg, type, onDone }) {
  ...
  const ok = type !== 'error'
  return (
    <div style={{ position:'fixed', bottom:24, right:24, padding:'11px 18px', display:'flex', alignItems:'center', gap:10, background:ok?'rgba(22,163,74,0.08)':'rgba(220,38,38,0.08)', border:`1px solid ${ok?'rgba(22,163,74,0.24)':'rgba(220,38,38,0.24)'}`, borderRadius:11, fontSize:13, fontWeight:500, color:ok?'#16a34a':'#dc2626', zIndex:9999, animation:'toastIn .28s ease both', boxShadow:'0 8px 32px rgba(0,0,0,0.12)', fontFamily:'inherit' }}>
      <span style={{ width:7, height:7, borderRadius:'50%', background:ok?'#16a34a':'#dc2626', flexShrink:0 }} />
      {msg}
    </div>
  )
}
...
export default function CreateTicketPage() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data: { session: s } }) => {
      ...
      setMacros(readMacrosFromStorage())
  ...
  useEffect(() => { if (ready) setTimeout(() => bodyRef.current?.focus(), 160) }, [ready])
  ...
  function insertLink() {
    ...
    bodyRef.current?.focus(); document.execCommand('createLink', false, url)
  }
  ...
  function applyMacro(m) {
    ...
    setBody(m.body); setMacroSearch(''); setShowMacroDD(false); bodyRef.current?.focus()
  }
  ...
  async function doSend() {
    ...
    if (!emailProvider) {
      ...
      setSending(false); setToast({ msg:'Message sent!', type:'success' })
      setTimeout(() => router.push('/inbox'), 700); return
    }
  ...
  return (
    <div className="ct-wrap" style={{ display:'flex', height:'100vh', overflow:'hidden', position:'relative' }}>
      <style>{CSS}</style>

      {/* Aurora (visible in dark mode only via CSS) */}
      <div aria-hidden style={{ position:'absolute', inset:0, overflow:'hidden', pointerEvents:'none', zIndex:0 }}>
        <div className="ct-al1" /><div className="ct-al2" /><div className="ct-grid" />
      </div>

      <Sidebar />

      {/* ═══ LEFT: Recent tickets ═══ */}
      <div className="ct-panel" style={{ width:308, flexShrink:0, display:'flex', flexDirection:'column', position:'relative', zIndex:1 }}>
        ...
        <div style={{ padding:'14px 14px 10px', borderBottom:'1px solid var(--border)', flexShrink:0 }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10 }}>
            ...
            <button
              ...
              style={{ display:'flex', alignItems:'center', gap:4, padding:'4px 10px', borderRadius:7, background:'var(--bg-input)', border:'1px solid var(--border)', color:'var(--text-2)', fontSize:11.5, fontWeight:600, fontFamily:'inherit', transition:'all .15s' }}
          ...
          <div style={{ fontSize:9.5, fontWeight:700, letterSpacing:'.1em', textTransform:'uppercase', color:'var(--text-3)' }}>Recent tickets</div>
        </div>
              ...
              <div key={t.id} className="ct-trow">
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:6, marginBottom:2 }}>
                    <span style={{ fontSize:11.5, fontWeight:600, color:'var(--text-1)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{t.from}</span>
                    <span style={{ fontSize:10, color:'var(--text-3)', flexShrink:0 }}>{t.time}</span>
                  </div>
                  <div style={{ display:'flex', alignItems:'center', gap:5 }}>
                    <span style={{ width:5, height:5, borderRadius:'50%', background:s.color, flexShrink:0 }} />
                    <span style={{ fontSize:11, color:'var(--text-2)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{t.subject}</span>
                  </div>
                </div>
              </div>
            )
      ...
      {/* ═══ RIGHT: Compose ═══ */}
      <div className="ct-compose" style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden', borderLeft:'1px solid var(--border)', position:'relative', zIndex:1 }}>
        ...
        <div style={{ borderBottom:'1px solid var(--border)', flexShrink:0 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', padding:'10px 16px', gap:8 }}>
            ...
            <div style={{ position:'relative', flexShrink:0 }}>
              <select
                ...
                style={{ appearance:'none', WebkitAppearance:'none', background:'var(--bg-input)', border:'1px solid var(--border)', borderRadius:6, padding:'3px 24px 3px 9px', fontSize:11.5, color:'var(--text-2)', fontFamily:'inherit', cursor:'pointer', outline:'none' }}
              >
                {PRIORITY_OPTS.map(p => <option key={p} value={p}>{p}</option>)}
              </select>
              <svg style={{ position:'absolute', right:7, top:'50%', transform:'translateY(-50%)', pointerEvents:'none' }} width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><polyline points="6 9 12 15 18 9"/></svg>
            </div>
            {/* Customer search */}
            <div style={{ display:'flex', alignItems:'center', gap:6, background:'var(--bg-input)', border:'1px solid var(--border)', borderRadius:8, padding:'5px 10px', width:210, flexShrink:0 }}>
              <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.3" style={{ color:'var(--text-3)', flexShrink:0 }}><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
            ...
            {/* Unassigned */}
            <button
              style={{ display:'flex', alignItems:'center', gap:5, padding:'4px 10px', background:'var(--bg-input)', border:'1px solid var(--border)', borderRadius:7, fontSize:11.5, color:'var(--text-2)', fontFamily:'inherit', whiteSpace:'nowrap', flexShrink:0, transition:'border-color .15s' }}
            ...
            {/* Close */}
            <button onClick={() => router.push('/inbox')} title="Close (Esc)"
              style={{ background:'none', border:'none', color:'var(--text-3)', padding:4, display:'flex', flexShrink:0, borderRadius:6, transition:'color .15s' }}
          ...
          {/* Row 2: Tags + metadata */}
          <div style={{ display:'flex', alignItems:'center', padding:'6px 16px', gap:14, fontSize:12, borderTop:'1px solid var(--border)', flexWrap:'wrap' }}>
            <div style={{ display:'flex', alignItems:'center', gap:6, flexWrap:'wrap' }}>
              {tags.map(t => (
                <span key={t} style={{ display:'inline-flex', alignItems:'center', gap:4, background:'var(--accent-soft)', border:'1px solid var(--accent-border)', borderRadius:5, padding:'1px 8px', fontSize:11.5, color:'var(--accent-text)' }}>
                  {t}
        ...
        {/* ── Compose bottom ── */}
        <div className="ct-compose-bottom" style={{ borderTop:'1px solid var(--border)', flexShrink:0 }}>

          {/* To */}
          <div className="ct-row">
            <span style={{ fontSize:10.5, fontWeight:700, color:'var(--text-3)', letterSpacing:'.09em', textTransform:'uppercase', width:40, flexShrink:0 }}>To</span>
          ...
          {/* CC + Bcc */}
          {showCC && (
            <div style={{ display:'flex', alignItems:'center', padding:'8px 16px', borderBottom:'1px solid var(--border)', gap:8, background:'var(--bg-input)' }}>
              <span style={{ fontSize:10.5, fontWeight:700, color:'var(--text-3)', letterSpacing:'.09em', textTransform:'uppercase', width:40, flexShrink:0 }}>CC</span>
              ...
              <span style={{ fontSize:10.5, fontWeight:700, color:'var(--text-3)', letterSpacing:'.09em', textTransform:'uppercase', width:40, flexShrink:0 }}>BCC</span>
          ...
          {/* Macro search */}
          <div style={{ display:'flex', alignItems:'center', padding:'7px 16px', borderBottom:'1px solid var(--border)', gap:8, position:'relative' }}>
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" style={{ color:'var(--text-3)', flexShrink:0 }}><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
            <input value={macroSearch} onChange={e => { setMacroSearch(e.target.value); setShowMacroDD(true) }}
              onFocus={() => setShowMacroDD(true)} onBlur={() => setTimeout(() => setShowMacroDD(false), 160)}
            ...
            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.3" style={{ color:'var(--text-3)' }}><polyline points="6 9 12 15 18 9"/></svg>
          ...
          {!body && suggested.length > 0 && (
            <div style={{ padding:'7px 16px 9px', borderTop:'1px solid var(--border)', display:'flex', alignItems:'center', gap:7, flexWrap:'wrap' }}>
              <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" style={{ color:'var(--text-3)', flexShrink:0 }}><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
          ...
          {/* Toolbar + Send */}
          <div style={{ display:'flex', alignItems:'center', padding:'8px 12px', borderTop:'1px solid var(--border)', gap:2 }}>
            ...
            <div style={{ flex:1 }} />
            <span style={{ fontSize:10.5, color:'var(--text-3)', marginRight:8 }}>⌘+Enter to send</span>
            <div style={{ display:'flex', alignItems:'stretch', borderRadius:9, overflow:'hidden', boxShadow:'0 2px 12px rgba(124,92,252,0.35)', flexShrink:0 }}>
              <button onClick={doSend} disabled={sending}
                style={{ display:'flex', alignItems:'center', gap:6, padding:'7px 18px', background:'var(--accent-gradient,linear-gradient(135deg,#7C5CFC 0%,#6d4af8 100%))', color:'#fff', border:'none', cursor:sending?'not-allowed':'pointer', fontSize:12.5, fontWeight:600, fontFamily:'inherit', opacity:sending?0.65:1, transition:'opacity .15s' }}>
                {sending
              ...
              <button onClick={doSend} disabled={sending}
                style={{ display:'flex', alignItems:'center', gap:5, padding:'7px 18px', background:'var(--accent-gradient,linear-gradient(135deg,#7C5CFC 0%,#6d4af8 100%))', color:'#fff', border:'none', cursor:sending?'not-allowed':'pointer', fontSize:12.5, fontWeight:600, fontFamily:'inherit', opacity:sending?0.65:1, transition:'opacity .15s', whiteSpace:'nowrap' }}>
                Send &amp; Close
              </button>
            </div>
          </div>
