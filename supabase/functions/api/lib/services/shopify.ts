...
interface ShopifyProductsResponse { products?: ShopifyProduct[] }
...
export async function getRefunds(credentials: ShopifyCredentials, dateRange: { from: string; to: string }) {
  ...
  while (nextUrl) {
    const page = await shopifyPaginatedFetch<ShopifyOrdersResponse>(credentials, nextUrl)
...
export async function syncOrders(
  ...
  while (url) {
    try {
      const page = await shopifyPaginatedFetch<ShopifyOrdersResponse>(credentials, url)
