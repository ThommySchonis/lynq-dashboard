...
const ITEMS = [
  ...
  },
  {
    ...
    icon: <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.85" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>,
  },
  {
    section: null,
            ...
            const inner = (
              <>
                <span className="sb-icon" title={item.label}>
                  <span style={{ color: iconColor, display:'flex', transition:'color .15s' }}>
                    {item.icon}
                  </span>
                  {item.badge && <span className="sb-badge" />}
                </span>
                <span className="sb-label" style={{ color: labelColor }}>
                  {item.label}
                </span>
            ...
            <div>
              ...
              <Link href="/admin" className={`sb-item${pathname.startsWith('/admin') ? ' sb-active' : ''}`}>
                <span className="sb-icon" title="Admin Panel">
                  <span style={{ color: pathname.startsWith('/admin') ? '#C4A0FF' : 'rgba(248,250,252,0.42)', display:'flex', transition:'color .15s' }}>
                ...
                <span className="sb-label" style={{ color: pathname.startsWith('/admin') ? '#F8FAFC' : 'rgba(248,250,252,0.62)' }}>
                  Admin Panel
                </span>
              </Link>
            </div>
