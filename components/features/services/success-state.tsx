...
export function SuccessState({ serviceName, onClose }: SuccessStateProps) {
  return (
    <div className="text-center py-3">
      ...
      <h3 className="text-[22px] font-extrabold text-(--text-1) tracking-[-0.04em] mb-2">
        Request sent!
      </h3>
      {serviceName && serviceName !== 'General Inquiry' && (
        <div className="inline-flex items-center px-3 py-1 rounded-full bg-(--bg-surface-2) border border-(--border) text-[12px] text-(--text-2) mb-3.5">
          {serviceName}
        </div>
      )}
      <p className="text-[14px] text-(--text-2) leading-[1.7] max-w-[320px] mx-auto mb-7">
        Your inquiry is with the Lynq & Flow team. We&apos;ll reach out within{' '}
        <strong className="text-(--text-2)">24 hours</strong>.
      </p>
