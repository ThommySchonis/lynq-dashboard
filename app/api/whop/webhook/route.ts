...
function getPlanFromMembership(membership: WhopMembership): string {
  const planId = membership?.plan_id || membership?.product_id
  return (planId && PLAN_MAP[planId]) || 'starter'
}

function verifyWhopSignature(body: string, signature: string, secret: string | undefined): boolean {
...
export async function POST(request: NextRequest) {
  ...
  let event: { action?: string; data?: { membership?: WhopMembership } & WhopMembership }
  try {
  ...
  const membership = (data?.membership || data) as WhopMembership | undefined
