...
export const TAG_PALETTE: Record<string, { bg: string; text: string; dot: string }> = {
...
export async function ensureTagsByName(supabaseAdmin: any, workspaceId: string, names: any[], createdBy?: string) {
  ...
  if (toCreateMap.size > 0) {
    const rows = Array.from(toCreateMap.values()).map((name: string) => ({
...
export async function syncMacroTags(supabaseAdmin: any, macroId: string, tagIds: string[]) {
  ...
  const rows = tagIds.map((tag_id: string) => ({ macro_id: macroId, tag_id }))
