...
export async function PATCH(request) {
  ...
  if ('locale' in body) {
    if (!['en', 'nl', 'de', 'fr', 'es'].includes(body.locale)) {
  ...
  if ('date_format' in body) {
    if (!['MM/DD/YYYY', 'DD/MM/YYYY', 'YYYY-MM-DD'].includes(body.date_format)) {
  ...
  if ('time_format' in body) {
    if (!['12h', '24h'].includes(body.time_format)) {
  ...
  if ('first_day_of_week' in body) {
    if (!['Sunday', 'Monday'].includes(body.first_day_of_week)) {
