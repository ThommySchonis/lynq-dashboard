...
export default function CreateTicketPage() {
  ...
  return (
    <div className="relative flex h-full overflow-hidden bg-(--bg-page)">
      {/* Aurora background (dark mode only, uses classes from globals.css) */}
      ...
      <div className="relative z-[1] flex w-[308px] shrink-0 flex-col border-r border-(--border) bg-(--bg-surface) dark:border-white/[0.07] dark:bg-[rgba(10,4,28,0.52)] dark:backdrop-blur-[24px]">
        {/* Header */}
        <div className="shrink-0 border-b border-(--border) px-3.5 pb-2.5 pt-3.5">
          <div className="mb-2.5 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="text-[15px] font-bold tracking-tight text-(--text-1)">Inbox</span>
              <span className="rounded border border-(--border) bg-(--bg-surface-2) px-1.5 py-px text-[10px] font-semibold text-(--text-2)">
                New ticket
              </span>
            </div>
            <button
        ...
        <div className="flex-1 overflow-y-auto">
          {DEMO_RECENT.map((t) => (
            <div
              key={t.id}
              className="flex cursor-pointer items-start gap-[9px] border-b border-(--border) px-3.5 py-2.5 transition-colors hover:bg-(--bg-surface-2)"
            >
              <ComposeAvatar name={t.from} size={28} />
              <div className="min-w-0 flex-1">
                <div className="mb-0.5 flex items-center justify-between gap-1.5">
                  <span className="truncate text-[11.5px] font-semibold text-(--text-1)">
                    {t.from}
                  </span>
                  <span className="shrink-0 text-[10px] text-(--text-3)">{t.time}</span>
                </div>
                <div className="flex items-center gap-[5px]">
                  <span className={`h-[5px] w-[5px] shrink-0 rounded-full ${STATUS_COLOR[t.status]}`} />
                  <span className="truncate text-[11px] text-(--text-2)">{t.subject}</span>
                </div>
              </div>
            </div>
          ))}
      ...
      {/* RIGHT: Compose area */}
      <div className="relative z-[1] flex flex-1 flex-col overflow-hidden border-l border-(--border) bg-(--bg-surface) dark:bg-[#160c35]">
        {/* Top bar */}
          ...
          <div className="flex flex-wrap items-center gap-3.5 border-t border-(--border) px-4 py-1.5 text-xs">
            <div className="flex flex-wrap items-center gap-1.5">
              ...
              <button
                ...
                className="inline-flex items-center gap-[3px] text-[11.5px] text-(--text-2)"
              >
            ...
            <span className="text-(--text-2)">
              Contact reason:{' '}
              <button className="text-(--text-2) text-xs">+Add</button>
            </span>
            <div className="h-[13px] w-px shrink-0 bg-(--border)" />
            <span className="text-(--text-2)">
              Product:{' '}
        ...
        <div className="flex flex-1 flex-col items-center justify-center gap-2.5 overflow-y-auto bg-(--bg-surface-2)">
          <div className="max-w-[380px] rounded-[14px] border border-(--border) bg-(--bg-surface) p-4 px-[22px] text-center shadow-(--shadow-card)">
            <Send className="mx-auto mb-2 h-[22px] w-[22px] text-(--text-3) opacity-70" />
            <div className="mb-[5px] text-[13px] font-semibold text-(--text-1)">
              New outgoing email
            </div>
            <div className="text-xs leading-[1.65] text-(--text-3)">
        ...
        {/* Compose bottom */}
        <div className="shrink-0 border-t border-(--border) bg-(--bg-surface) dark:bg-[#120931]">
          {/* To */}
          ...
          {accountInfo?.email && (
            <div className="flex items-center gap-2 border-b border-(--border) px-4 py-[9px]">
              ...
              <div className="flex items-center gap-1.5">
                <Mail className="h-[13px] w-[13px] text-(--text-3)" />
                <span className="text-[13px] text-(--text-2)">{accountInfo.email}</span>
              </div>
            </div>
          )}
            ...
            <div className="flex items-center gap-[7px] border-b border-(--border) bg-amber-500/5 px-4 py-[7px] dark:bg-amber-400/[0.06]">
              ...
              <span className="text-[11.5px] text-amber-600">
                ...
                <a
                  ...
                  className="font-semibold text-(--text-1) no-underline"
                >
          ...
          {/* CC + BCC */}
          {showCC && (
            <div className="flex items-center gap-2 border-b border-(--border) bg-(--bg-input) px-4 py-2">
              <span className="w-10 shrink-0 text-[10.5px] font-bold uppercase tracking-[0.09em] text-(--text-3)">
                CC
              </span>
              <input
          ...
          <div className="relative flex items-center gap-2 border-b border-(--border) px-4 py-[7px]">
            ...
            {showMacroDD && macroHits.length > 0 && (
              <div className="absolute bottom-[calc(100%+4px)] left-0 right-0 z-[60] max-h-[220px] animate-[fadeUp_0.14s_ease_both] overflow-y-auto rounded-[10px] border border-(--border) bg-(--bg-surface) p-1 shadow-[0_-12px_32px_rgba(0,0,0,0.1)] dark:bg-[#1a0b3d] dark:shadow-[0_-12px_32px_rgba(0,0,0,0.4)]">
                {macroHits.map((m) => (
                  <button
                    key={m.id}
                    className="block w-full cursor-pointer rounded-[7px] border-none bg-none px-[11px] py-2 text-left transition-colors hover:bg-(--bg-surface-2)"
                    onMouseDown={() => applyMacro(m)}
                  >
                    <div className="text-[12.5px] font-semibold text-(--text-1)">
                      {m.name}
                    </div>
                    <div className="mt-px truncate text-[11.5px] text-(--text-2)">
                      {m.body?.replace(/\n/g, ' ')}
                    </div>
                  </button>
                ))}
          ...
          <div
            ...
            className="min-h-[120px] w-full resize-none overflow-y-auto border-none bg-transparent px-4 py-3.5 text-[13.5px] leading-[1.78] tracking-[0.005em] text-(--text-1) outline-none empty:before:pointer-events-none empty:before:block empty:before:text-(--text-3) empty:before:content-[attr(data-placeholder)]"
          />

          {/* Suggested macros */}
          {!body && suggested.length > 0 && (
            <div className="flex flex-wrap items-center gap-[7px] border-t border-(--border) px-4 py-[7px] pb-[9px]">
              <Clock className="h-[11px] w-[11px] shrink-0 text-(--text-3)" />
              <span className="text-[11px] font-medium text-(--text-3)">Suggested macros</span>
              {suggested.map((m) => (
                <button
                  key={m.id}
                  onClick={() => applyMacro(m)}
                  className="rounded-full border border-(--border) bg-(--bg-surface) px-2.5 py-[2px] text-[11.5px] text-(--text-1) shadow-(--shadow-row) transition-colors hover:border-(--border-hover)"
                >
                  {m.name}
                </button>
              ))}
          ...
          <div className="flex items-center gap-0.5 border-t border-(--border) px-3 py-2">
            <button
              className="flex h-[30px] min-w-[26px] items-center justify-center rounded-[7px] px-1.5 text-xs font-bold text-(--text-3) transition-colors hover:bg-(--bg-surface-2) hover:text-(--text-1)"
            ...
            <button
              className="flex h-[30px] min-w-[26px] items-center justify-center rounded-[7px] px-1.5 text-xs italic text-(--text-3) transition-colors hover:bg-(--bg-surface-2) hover:text-(--text-1)"
              onMouseDown={(e) => e.preventDefault()}
              onClick={() => fmt('italic')}
              title="Italic"
            >
            ...
            <button
              className="flex h-[30px] min-w-[26px] items-center justify-center rounded-[7px] px-1.5 text-xs text-(--text-3) underline transition-colors hover:bg-(--bg-surface-2) hover:text-(--text-1)"
              onMouseDown={(e) => e.preventDefault()}
            ...
            <button
              className="flex h-[30px] min-w-[30px] items-center justify-center rounded-[7px] px-1.5 text-(--text-3) transition-colors hover:bg-(--bg-surface-2) hover:text-(--text-1)"
              onMouseDown={(e) => e.preventDefault()}
              onClick={insertLink}
              title="Link"
            >
