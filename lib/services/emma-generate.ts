...
export interface EmmaMessageInput {
  from?: string
  date?: string
  body?: string
  snippet?: string
...
export interface GenerateEmmaDraftParams {
  workspaceId: string
  userId: string
  userEmail: string
  memberId: string | null
  conversationId: string | undefined
  messages: EmmaMessageInput[]
  language?: string
  /** When false, the autonomy/auto-send gate is skipped entirely → draft is always status='pending'. */
  allowAutoSend: boolean
}

export interface GenerateEmmaDraftResult {
  replyText: string
  draftId: string | null
  autoSent: boolean
  autoSendBlockedReason: AutoSendBlockedReason | null
}
...
export async function generateEmmaDraft(
  params: GenerateEmmaDraftParams,
): Promise<GenerateEmmaDraftResult> {
  const { workspaceId, userId, userEmail, memberId, conversationId, messages, language, allowAutoSend } = params
  ...
  try {
    storeId = await resolveStoreIdForThread(conversationId, workspaceId)
    if (storeId) {
      const onboarding = await getOnboardingStatus(storeId, workspaceId)
      if (onboarding.isComplete && onboarding.policies) {
        ...
        try {
          lessons = await getEnabledLessons(storeId, workspaceId)
        } catch (err) {
          logger.error('[ai/reply]', 'lessons load failed', err)
        }
        let examples: AiExample[] = []
        try {
          examples = await getExamples(storeId, workspaceId)
        } catch (err) {
...
Write only the reply body. Do not include subject lines, metadata, or explanations. Sign off as "${brandName}".`
    ...
    if (!autoSentAt && draftId) {
      try {
        await supabaseAdmin.from('notifications').insert({
          ...
          link:        `/inbox?conversation_id=${conversationId}`,
  ...
  return { replyText, draftId, autoSent: Boolean(autoSentAt), autoSendBlockedReason }
}
